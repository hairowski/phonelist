...
class ICal
{
	...
	public function formatIcalDate($icaldt){
	
		$year = substr($icaldt,0,4);
		$month = substr($icaldt,4,2);
		$day = substr($icaldt,6,2);
		$date = $day.'/'.$month.'/'.$year;
	...
	public function formatIcalTime($icaldt){
	
		$hours = substr($icaldt,-6,2);		
		$minutes = substr($icaldt,-4,2);
		$time = $hours.':'.$minutes;
    ...
    public function eventsFromRange($rangeStart = false, $rangeEnd = false) 
    {
        ...
        if ($rangeEnd !== false ) { //if y else originales estaban girados...
		...
		//degbug!!
		//echo $rangeStart.' '.$rangeEnd; exit;
		
        

        // loop through all events by adding two new elements
        foreach ($events as $anEvent) {
            $start = $this->iCalDateToUnixTimestamp($anEvent['DTSTART']);
			$end = $this->iCalDateToUnixTimestamp($anEvent['DTEND']);
			$duration = $end - $start;
			//debug!!
			//echo $timestamp.' >= '.$rangeStart.' AND '.$timestamp.'<='.$rangeEnd.'<br/>';
			//echo $anEvent['DTSTART'];
            if ((($start + $duration) >= $rangeStart) && (($start + $duration) <= $rangeEnd)) {
